...
## Структура проекта
```markdown
[calculate_project](https://github.com/syirnik/GO_Yandex)
...
### Объяснение:
- **Название репозитория**: В структуре проекта в README.md ты указываешь название проекта, а не путь до репозитория. Когда ты клонируешь репозиторий на локальную машину, он будет сохранён в папке с именем репозитория.
...
Теперь можно отправлять запросы к API для вычисления арифметических выражений.
...
curl -s -X POST http://localhost:8080/api/v1/calculate -d '{"expression": "3 + 5 * (2 - 8)"}'
```
На обеих системах синтаксис одинаков и использует стандартные кавычки.
